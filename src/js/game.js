...
const checkLetter = (letter) => {
  ...
  // есди буквы нет в загаданном слове
...
const stopGame = (status) => {
  ...
  if (status === "win") {
    ...
    // сценарий проигрыша
    document.getElementById("game").innerHTML +=
      '<h2 class="result-header lose text-center">You lost :(</h2>';
  } else if (status === 'quit') {
  ...
  document.getElementById("game").innerHTML += 
  `<p>The word was: <span class="result-word">${word}</span></p><button id="play-again" class="button-primary px-5 py-2 mt-5">Play again</button>`;
